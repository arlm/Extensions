...
namespace System.Threading.Tasks.Schedulers
{
    ...
    public sealed class IOCompletionPortTaskScheduler : TaskScheduler, IDisposable
    {
        ...
        private sealed class IOCompletionPort : IDisposable
        {
            ...
            private uint INFINITE_TIMEOUT = unchecked((uint)Timeout.Infinite);
            ...
            public IOCompletionPort(int maxConcurrencyLevel)
            {
                ...
                m_handle = CreateIoCompletionPort(INVALID_FILE_HANDLE, INVALID_IOCP_HANDLE, UIntPtr.Zero, (uint)maxConcurrencyLevel);
            }
            ...
            public void NotifyOne()
            {
                ...
                if (!PostQueuedCompletionStatus(m_handle, 0, IntPtr.Zero, ref lpOverlapped))
                {
            ...
            public bool WaitOne()
            {
                ...
                if (!GetQueuedCompletionStatus(m_handle, out lpNumberOfBytes, out lpCompletionKey, out lpOverlapped, INFINITE_TIMEOUT))
                {
            ...
            private static extern bool GetQueuedCompletionStatus(SafeFileHandle completionPort, out uint lpNumberOfBytes, out IntPtr lpCompletionKey, out IntPtr lpOverlapped, uint dwMilliseconds);
            ...
            [DllImport("kernel32.dll", SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            private static extern bool PostQueuedCompletionStatus(SafeFileHandle completionPort, uint dwNumberOfBytesTransferred, IntPtr dwCompletionKey, ref NativeOverlapped? lpOverlapped);
        }
    }
}